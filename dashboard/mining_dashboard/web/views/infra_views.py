...
Split out of ``web/views/views.py`` (#1105): the host system gauges (Issue #32), the per-worker rig rows
...
``web/views/worker_detail.py`` imports the two RigForge helpers from there — so the move is invisible to
consumers.
...
from mining_dashboard.service.health.update_checker import compute_update
from mining_dashboard.web.views.rigforge_views import _num
from mining_dashboard.web.views.rigforge_views import rigforge_display as _rigforge_display
