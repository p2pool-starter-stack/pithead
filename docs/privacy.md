...
| **Tari** P2P | Tari network | — | ✅ Tor (`type = "tor_tcp"`: onion peers, and IP peers through a Tor exit) | on | Tor by default; can opt into clearnet (TCP) for the initial sync only ([#183](#optional-clearnet-initial-sync-off-by-default)) |
...
| **Tari** transport | Tor (`type = "tor_tcp"`) | **TCP** (`type = "tcp"`) |
