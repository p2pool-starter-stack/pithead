...
- **Shared data root** — point `monero.data_dir`, `tari.data_dir`, `p2pool.data_dir`, and
  ...
  database arrived. An explicit `dashboard.data_dir` you set is never moved for you: the run
  warns and leaves both directories alone, so move it yourself with the stack down to keep
  history and the payout-wallet alarm baseline. Data at *both* locations stops the run
  rather than guessing which database is live.
