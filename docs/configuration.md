...
| `monero.rpc_lan_access` | `false` | `true` publishes the node's RPC on the LAN (`0.0.0.0`) for wallets on other machines; default is localhost-only. See [Connecting a Wallet](wallets.md) for the credentials, the address to use, and the security tradeoffs. |
