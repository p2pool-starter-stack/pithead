...
| `tari.auto_restart` | `true` | Restart the local Tari node when the dashboard's chain verdict stays red: its tip unchanged for 30 minutes with 0 peers for 10, or its height more than 50 blocks behind `tari.explorer_url` (#2464). The dashboard waits for 5 minutes of red, then restarts at most 3 times per outage, an hour apart, never while the node's gRPC is not answering (a database migration may be running). After the third restart the advice says a fork or a required upgrade is likely. `false` leaves it to you; the panel, doctor, status and the alert report the verdict either way. Local mode only. Changeable from the dashboard's Configuration view. See [Operations › Troubleshooting](operations.md#troubleshooting). |
| `tari.explorer_url` | `https://textexplore.tari.com/?json` | The public Tari tip the verdict compares against, fetched through Tor once an hour. It is the one reference that is not the node's own opinion, so it is what catches a node on a dead fork. The URL must return the text explorer's JSON (`tipInfo.metadata.best_block_height`). Blank turns the reference off, and a failed fetch contributes nothing, so a missing reference never makes the verdict red. See [Privacy](privacy.md). |
