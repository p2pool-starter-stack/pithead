...
| `monero.remote.host` / `rpc_port` / `zmq_port` | — / `18081` / `18083` | Remote node connection details (used when `mode` is `remote`). Changeable from the dashboard's Configuration view behind a typed `APPLY`, and only if the host can reach the new endpoint ([#1888](https://github.com/p2pool-starter-stack/pithead/issues/1888)); node credentials use the same confirmation path and stay masked in previews. |
...
| `dashboard.data_dir` | `auto` | Where the dashboard's database lives. `auto` = `./data/dashboard`, unless the four other `*.data_dir` all point under one parent directory — then the dashboard joins them at `<that parent>/dashboard`. `upgrade`/`apply` moves the live database only from the old in-install default; an explicit path you set is yours, and the run warns and leaves both directories alone. Move it yourself with the stack down to keep history and the payout-wallet alarm baseline. Data at both paths stops the move rather than guessing (see [Data directories](#data-directories)). |
