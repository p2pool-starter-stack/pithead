...
|---|---|---|
| **Mining** |  | _Choose how the pool, proxy, local miner, and XvB route mining work._ |
...
| `monero.payout_scan_height` | `auto` | Block height the view-only wallet starts scanning from on first creation (#381). `auto` = genesis (height `0`), so it scans the full payout history; this first scan can take hours. Set a later height to skip older payouts and shorten the first scan. Only affects the first wallet creation; ignored once the wallet file exists. |
| `tari.wallet_address` | _required unless `tari.mode` is `off`_ | Your Tari (Minotari) payout address, in either the base58 or the emoji form (single or dual, embedded payment id supported). Both forms carry a built-in checksum, and it is verified — a single mistyped character fails it, which otherwise means merge-mine rewards silently lost. A checksum-valid address for a Tari testnet is rejected too: the stack mines mainnet. `setup`/`apply` reject both cases. |
...
| `monero.node_username` / `node_password` | _auto (local)_ | Credentials for the local node's RPC. Leave them blank: on `setup` and on every `apply`, the stack fills in anything missing (username `admin`, a random alphanumeric password) and writes the values back into `config.json`, so they stay stable and you can see what was set. Used inside the stack (monerod, p2pool, and the dashboard, which reads the node's `get_info` for sync status) — and, once `monero.rpc_lan_access` is on, these are also what an external wallet must present to the node's RPC; see [Connecting a Wallet](wallets.md). Set these yourself only for a remote node that requires RPC auth; a remote node without RPC auth leaves both fields empty. |
...
| `monero.clearnet_initial_sync` | `false` | Privacy-relevant, default off. Takes effect only with `network.tor_egress_firewall: false`: with the firewall on, the flag is ignored and the node syncs over Tor. `true` makes monerod do its initial block download over clearnet (much faster than Tor) by dropping the Tor P2P `proxy=` and lowering `out-peers` to 32. Transaction broadcast stays on Tor and wallets are never exposed. Your node's IP becomes visible to the Monero P2P network while it's on, and `pithead` warns loudly (apply/status/doctor/up) the whole time. The dashboard switches monerod back to Tor automatically once the chain is synced (#234) and keeps it there, so you can leave this `true`. Applies to the bundled node only — with `monero.mode: remote` nothing here acts on it, so set it back to `false` before switching. Full threat model: [Privacy › Optional clearnet initial sync](privacy.md#optional-clearnet-initial-sync-off-by-default). |
| `monero.rpc_lan_access` | `false` | `true` publishes the node's RPC on the LAN for wallets on other machines; default is localhost-only. LAN means the source address: see [LAN-only sources](#lan-only-sources). See [Connecting a Wallet](wallets.md) for the credentials, the address to use, and the security tradeoffs. |
| `monero.zmq_lan_access` | `false` | `true` publishes the node's ZMQ block-notification feed (`18083`) on the LAN. With `rpc_lan_access`, this is the serving side of `monero.mode: remote` — a remote P2Pool needs both RPC and ZMQ. ZMQ has no authentication: trusted networks only, and only LAN sources can connect (see [LAN-only sources](#lan-only-sources)). In `local` mode the port is published either way — on `127.0.0.1` while this is `false` — so free `18083` on the host if something else holds it, or the container won't start. Ignored in `remote` mode (no bundled node runs). |
...
| **Tari node** |  | _Choose the Tari node and how Pithead reaches or exposes it._ |
| `tari.mode` | `local` | `local` runs the bundled Tari base node; `remote` merge-mines against an external one (see `tari.remote` and [Remote Tari node](#remote-tari-node)); `off` declines Tari altogether: no Tari container, no `--merge-mine` arguments to P2Pool, and no Tari payout address needed. Both setup wizards ask this outright and write the key explicitly whichever way it is answered ([#1916](https://github.com/p2pool-starter-stack/pithead/issues/1916)); `./pithead setup` offers `local` or `off` as the default according to whether the filesystem holding `./data` has room for the bundled node's chain, and prints the figures behind that. Changeable from the dashboard's Configuration view behind a typed `APPLY` ([#1929](https://github.com/p2pool-starter-stack/pithead/issues/1929)) — switching to `off` stops the node and the merge-mining but **keeps the chain data on disk**, so switching back resumes instead of re-syncing. Switching a machine that has no `tari.wallet_address` to `local`/`remote` needs that address too, which is an approval-step change. A config that omits this key still means `local`, so upgrading never silently stops merge-mining. |
| `tari.clearnet_initial_sync` | `false` | Privacy-relevant, default off. Takes effect only with `network.tor_egress_firewall: false`: with the firewall on, the flag is ignored and the node syncs over Tor. `true` makes the Tari base node sync over clearnet instead of Tor: it switches the P2P transport to TCP, re-enables the `seeds.tari.com` DNS seed (the bundled onion `peer_seeds` are unreachable without Tor), and stops advertising its onion. Your node's IP becomes visible to the Tari P2P network while it's on, plus one DNS lookup of `seeds.tari.com`. `pithead` warns loudly (apply/status/doctor/up). The dashboard switches Tari back to Tor automatically once it's synced (#234), so you can leave this `true`. Applies to the bundled node only — with `tari.mode: remote` nothing here acts on it, so set it back to `false` before switching. Full threat model: [Privacy › Optional clearnet initial sync](privacy.md#optional-clearnet-initial-sync-off-by-default). |
...
| `tari.grpc_lan_access` | `false` | `true` publishes the local Tari base node's gRPC (`18142`) on the LAN — the serving side of `tari.mode: remote`, so other stacks can merge-mine against this node. It's for merge-mining by another stack, not for wallets: connecting a wallet to the bundled Tari node isn't a supported use case (the node's gRPC allowlist drops the wallet/transaction and UTXO/kernel search calls). The gRPC is plaintext and unauthenticated: trusted networks only, and only LAN sources can connect (see [LAN-only sources](#lan-only-sources) and [Remote Tari node](#remote-tari-node)). Ignored in `remote` mode (no bundled node runs). In `local` mode the port is published either way — on `127.0.0.1` while this is `false` — so free `18142` on the host if something else holds it, or the container won't start. |
| **Workers** |  | _Control how rigs authenticate and connect to Pithead._ |
| `workers.api_auth` | `none` | How the dashboard reads each worker's xmrig API. Beyond what the proxy reports, the dashboard probes every connected miner's own xmrig HTTP API (`/1/summary`) for uptime and per-miner hashrate, one configured way, no auto-detection. `none` (default) expects an open, read-only API (xmrig `http.restricted` with no `access-token`), which is what a stock RigForge worker exposes. `name` sends the worker's stratum name as the Bearer token (for miners whose `access-token` equals their name). `token` sends a single shared `workers.api_token` for every worker. A worker whose probe fails isn't dropped: it keeps its proxy-reported hashrate and is badged on the dashboard, with one log line explaining why (so a misconfigured API is distinct from an offline miner). An _adopted_ rig — one whose `workers.list` entry carries a control token — is badged `api ⚠`, the case this key is about; a rig with no token is badged `not adopted` instead — the setup wizard's rig, whose fix is adopting it; a hand-configured miner in that state still wants this key (#1857). Upgrading from a build whose miners set an `access-token`? Set this to `name` (or reprovision the miners to drop the token), otherwise the default no-auth probe `401`s and every worker is badged — `api ⚠` if adopted, `not adopted` if not. |
...
| **Energy** |  | _Set electricity and price inputs for cost estimates._ |
...
| **Alerts** |  | _Set the thresholds that turn mining changes into alerts._ |
...
| **Advanced** |  | _Tune storage, memory, networking, Tor, and update behavior._ |
...
`monero.rpc_lan_access`, `monero.zmq_lan_access` and `tari.grpc_lan_access` publish `18081`,
`18083` and `18142` on every IPv4 interface of the host. A firewall rule that `pithead` installs
...
[Tailscale](https://tailscale.com/) (a `100.64.0.0/10` address). Exposing them to public
