...
| `tari.data_dir` | `auto` | Where the Tari node data lives on the host. `auto` = `./data/tari`. Unused with `tari.mode: remote` or `off` — no local node runs, and `setup`/`doctor` drop Tari's ~200 GB from this host's disk budget. Switching to `off` does not delete it, so switching back resumes from the chain already there. See [Hardware › Running a node elsewhere](hardware.md#running-a-node-elsewhere). |
