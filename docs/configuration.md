...
| `dashboard.data_dir` | `auto` | Where the dashboard's database lives. `auto` = `./data/dashboard`, unless the four other `*.data_dir` all point under one parent directory — then the dashboard joins them at `<that parent>/dashboard`. `upgrade`/`apply` moves the live database whenever this path changes, preserving history and the payout-wallet alarm baseline; data at both paths stops the move rather than guessing (see [Data directories](#data-directories)). |
