...
| `monero.remote.host` / `rpc_port` / `zmq_port` | — / `18081` / `18083` | Remote node connection details (used when `mode` is `remote`). These fields and `node_username` / `node_password` are changeable from the dashboard's Configuration view behind a typed `APPLY`. Before committing an endpoint or login change, the host authenticates to the staged endpoint with the staged login ([#1888](https://github.com/p2pool-starter-stack/pithead/issues/1888)). On a local node, changing either login field preserves its masked partner and applies the pair together to monerod, P2Pool and the dashboard. |
