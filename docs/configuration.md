...
| `tari.mode` | `local` | `local` runs the bundled Tari base node; `remote` merge-mines against an external one (see `tari.remote` and [Remote Tari node](#remote-tari-node)); `off` declines Tari: no Tari container, and no Tari payout address needed. P2Pool is still handed its merge-mine arguments until #1903 lands, so today `off` stops the node rather than the merge-mining itself. A config that omits this key still means `local`, so upgrading never silently stops merge-mining. |
