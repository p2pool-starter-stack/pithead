...
with the stated IPv4-TCP/bridge-network limit, not a residual gap. Covered without a real chain (tiers
...
| Fault injection over SSH: no recorded live evidence | Low-Medium. Jobs 631 and 702 selected the focused phase but the pre-fault lifecycle restore did not return the stack healthy, so they are not proof; [#2501](https://github.com/p2pool-starter-stack/pithead/issues/2501) tracks that recovery defect and [#2000](https://github.com/p2pool-starter-stack/pithead/issues/2000) tracks the focused remote quoting, cleanup, and restoration proof. |
