...
| P4 | Only if P2 keeps #2057, #1997 and #2473 in 2.0.0: PR #2180 merged, with its `image-upgrade` rows passing on a tier4-kvm job | PR #2180 is a draft; its phase ran on tier4-kvm and failed the candidate-trust and safety-rollback checks [r PR #2180; #2473]. `tests/os/phases/` on `develop` has no image-upgrade phase [r]. bench-ci#560 is a maintainer pointer [i] |
...
| 7 | Publish; post-publish smoke, including the upgrade from 1.20.0 | `make release-smoke` against the tag | docs/dev/releasing.md:441-452, :468-471 |
...
| #2057, #1997, #2473, if P2 moves them | the image-upgrade gate, which upgrades a v1.20.0 install to the submitted commit [r PR #2180]; it runs on tier4-kvm through PR #2180 (draft) and has not passed yet [r PR #2180; #2473] |
...
| R3 | #2057, #1997, #2473 (the image-upgrade gate): in 2.0.0 or 2.0.x | 2.0.x, because the gate has not passed yet. The trade: §2.4 step 7's post-publish smoke is the first proof that the 1.20.0 → 2.0.0 upgrade completes [r docs/dev/releasing.md:468-471], and nothing before 2.0.x proves that chains and secrets survive it. Keep PR #2180 moving either way |
