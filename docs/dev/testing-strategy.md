...
| Unmatched-Host catch-alls own :80 (#1123) and :443 (#1132's third bullet) instead of leaving Caddy's own defaults — a reflecting redirect on :80, a silent empty 200 on :443 — to answer a forged Host or SNI: both redirect to the box's own canonical address (never one built from the request), both carry the same bind list as the named vhosts, and the :443 one carries no `tls` line of its own. Every Caddyfile generated by the existing DIY, appliance identity, auth, custom-port and onion render scenarios is adapted by the real Caddy parser from the image digest Compose ships; this catches syntax and structural errors but does not provision TLS or open sockets | `tests/stack/dashboard/test-dashboard.sh`, `tests/stack/dashboard/test-dashboard-onion.sh`, `tests/stack/appliance/test-appliance-caddyfile-optional-env.sh`, `tests/stack/appliance/test-appliance-identity.sh` (sourced `generate_caddyfile`) + `tests/stack/standalone/test_caddyfile.sh` (`caddy adapt`) | 1 ✅ |
