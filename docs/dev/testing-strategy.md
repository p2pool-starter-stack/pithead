...
| HugePages each daemon actually holds (#2685): monerod and p2pool are sampled every 10 s for the whole destructive run from `/proc/<pid>/smaps_rollup` (`Private_Hugetlb` + `Shared_Hugetlb`), never from a `HugePages_Free` delta, which other users of the pool pollute. Red when a daemon held no hugetlb page at any reading, or when any instance of it ran 300 s without one (the #78 fallback); the peaks land in `results/hugepages-peak.json` as the measurement the appliance's reduced tier is sized from | live stack, every destructive run | 1 ✅ (tally, gate, artifact and the `/proc` read against a stand-in process, `selftest-hugepage-probe.sh`) · 4 ▶ |
