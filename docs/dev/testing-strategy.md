...
| Wizard's real HTTP flow provisions the STACK (images verified, direct clearnet egress refused and Tor-routed egress succeeds from the Monero container through its shipped `/usr/bin/curl`, a direct IPv6 dial refused whenever that container holds a global v6 address, as read from `/proc/net/if_inet6` with the image's `/usr/bin/cat` since the image ships no `ip`, miner up); a probe that cannot run, whether curl or the v6 guard's cat, is a counted unverified failure and never the IPv4-only pass; unaided reboot return; after each of M10's three live-stack power cuts every pre-cut container, stored image digest, non-regressing monerod height, miner, Caddy, and committed slot return; commit-gate honesty both ways; the migration hold starts the chain only post-commit | battery `--phase provision` | 4 ✅ |
