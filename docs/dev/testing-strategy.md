...
| Restore leg: the checked-in encrypted v1.20.0 fixture restores to a keep-preserved target; wallet, Tor identity, RPC/dashboard/onion secrets, and both fixture and target chain sentinels survive without a resync (#2001/#2230). A carried archive also clears only the target's superseded config and role markers, so firstboot consumes it while the chain trees remain intact | `tests/stack/appliance/test-appliance-install-restore.sh` (real installer block) · battery `--phase install` | 1 ✅ · 4 ✅ |
