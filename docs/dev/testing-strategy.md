...
| `monero.clearnet_initial_sync` / `tari.clearnet_initial_sync` (#183/#234/#2649): a flag reaches the daemons only while the egress firewall is off; the node then syncs over clearnet, switches itself back to Tor, and stays there when the firewall comes back on | config → `.env` → live daemon | 1 ✅ (render and marker re-arm, `test-clearnet-firewall.sh`; the harness's firewall-restore step, `selftest-clearnet-firewall.sh`) · 4 ▶ (matrix `local-pruned-main-clearnet-sync`, the one scenario run with the firewall off) |
