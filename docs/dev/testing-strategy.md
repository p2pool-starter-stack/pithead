...
| RC2 boot-menu version labels (#1956): serial sees distinct current/previous slot versions after the update commits, with the final boot bounded to bytes emitted after the commit reboot | battery `--phase update` serial assertion from #1956 | 1 ✅ (menu fixtures) · 4 (assertion written in #1956; unverified until that branch is integrated into the image and the battery runs) |
