...
| `rotate-dashboard-onion` (#2345): the new address answers over real Tor, the retired address stops answering, the Caddyfile drops the old vhost (#546), `HOST_IP` survives the render (#356), and the client-auth key changes | real box, real Tor | 1 ✅ (`tests/stack/dashboard/test-dashboard-onion.sh`, stubbed engine) · 4 ▶ (`--rotate-onion`) |
