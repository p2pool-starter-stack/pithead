...
| Tari's active chain is pinned to `mainnet` (#2304): `build/tari/config.toml.template` explicitly sets `network = "mainnet"`, so `minotari_node` never falls back to its binary-default network and silently syncs a different chain than the `[mainnet.*]` sections assume; the pin is asserted on both the canonical template and the entrypoint's rendered runtime config | config → render | 1 ✅ (`tests/stack/test-wizard-tari.sh`) |
