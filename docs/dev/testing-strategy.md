...
| Wizard's real HTTP flow provisions the STACK (images verified, direct clearnet egress refused and Tor-routed egress succeeds from the Monero container through its shipped `/usr/bin/curl`, a direct IPv6 dial refused whenever that container holds a global v6 address, as read from `/proc/net/if_inet6` with the image's `/usr/bin/cat` since the image ships no `ip`, miner up); a direct dial counts as dropped only when it times out, and a probe that cannot run (curl, the v6 guard's cat, or a dial that fails any other way) is a counted unverified failure, never a pass; unaided reboot return; after each of M10's three live-stack power cuts every pre-cut container, stored image digest, non-regressing monerod height, miner, Caddy, and committed slot return; commit-gate honesty both ways; the migration hold starts the chain only post-commit | `tests/os/appliance-egress-leg.sh --self-test` (stubbed guest: missing curl or cat, dial rc, v6 scope) · battery `--phase provision` | 1 ✅ · 4 ✅ (the egress probes changed for bench-ci#532 — unverified until the next battery run) |
