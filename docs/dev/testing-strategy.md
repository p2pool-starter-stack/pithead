...
| `tari.mode` local / remote / off (#103/#1855/#1916): tari container/onion present/absent, sync gate against a remote target, an `off` machine that runs no Tari and merge-mines nothing while still mining Monero, and both setup wizards writing the key explicitly — including `./pithead setup` declining Tari and still provisioning | config | 1 ✅ (harness logic, `selftest-tari-mode-off.sh`; the CLI wizard's question and its disk-derived default, `tests/stack/test-wizard-tari.sh`) · 4 ▶ (live matrix) |
