...
| `tari.mode` local / remote / off (#103/#1855): tari container/onion present/absent, sync gate against a remote target, and an `off` machine that runs no Tari and merge-mines nothing while still mining Monero | config | 1 ✅ (harness logic, `selftest-tari-mode-off.sh`) · 4 ▶ (live matrix) |
