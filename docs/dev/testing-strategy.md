...
| A live Tari node stranded from its peers stops reading healthy, and recovers (#2464). A host `DOCKER-USER` rule tagged `pithead-e2e-fault-tari-stranded` drops the tari container's traffic to tor. The leg asserts the dashboard verdict goes amber within 10 minutes plus a poll, then red with a non-zero doctor exit and a red `pithead status` line within 30 minutes plus a poll. It freezes tari (SIGSTOP, standing in for a migration) and asserts the automatic restart is withheld. It then removes the rule, asserts the restart fires and the verdict returns to green after catch-up. The rule is also removed by an EXIT trap, and the leg asserts none is left | a local Tari node, passwordless `sudo iptables` on the box | 1 ✅ (verdict, guards and alerts: `test_tari_health.py`; doctor/status: `tests/stack/test-tari-chain.sh`) · 4 ▶ (`run.sh --tari-stranded`; bench-ci phase `tari-stranded`, about an hour, opt-in) |
