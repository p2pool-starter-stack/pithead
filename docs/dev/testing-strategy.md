...
| Restore-at-setup (#909): encrypted and plaintext archives, volatile secrets and plaintext, TLS-only upload, serialized publication, retry refusal, failed cleanup, bad passphrase, oversize and malformed fallback. Server/client tests cover the transport contract; the shell suites apply a genuine backup, enforce numeric-owner and expansion limits, prove volatile submission/staging/installer carry and restart cleanup, and preserve target files on chain-data collisions (#2195). | `dashboard/tests/web/test_wizard_handoff_rig_and_restore.py` + `dashboard/tests/web/test_wizard_restore_limits.py` + `dashboard/tests/web/test_wizard_restore_security.py` + `dashboard/tests/frontend/wizard/wizard-submit.test.mjs` + `tests/stack/appliance/test-appliance-restore.sh` + `tests/stack/appliance/test-appliance-restore-reject.sh` + `tests/stack/appliance/test-appliance-wizard-spool.sh` | 1 ✅ |
