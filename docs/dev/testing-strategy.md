...
| Restore-at-setup (#909): encrypted and plaintext archives, volatile secrets and plaintext, TLS-only upload, serialized publication, retry refusal, failed cleanup, bad passphrase, oversize and malformed fallback. Server/client tests cover the transport contract; the shell suites apply a genuine backup, enforce numeric-owner and expansion limits, prove volatile submission/staging/installer carry and restart cleanup, and preserve target files on chain-data collisions (#2195). A failure injected inside the commit (the dashboard swap, the chain merge, the rollback itself) puts back every item already replaced and removes the names the merge added (#2689). | `dashboard/tests/web/test_wizard_handoff_rig_and_restore.py` + `dashboard/tests/web/test_wizard_restore_limits.py` + `dashboard/tests/web/test_wizard_restore_security.py` + `dashboard/tests/frontend/wizard/wizard-submit.test.mjs` + `tests/stack/appliance/test-appliance-restore.sh` + `tests/stack/appliance/test-appliance-restore-legacy.sh` + `tests/stack/appliance/test-appliance-restore-publish.sh` + `tests/stack/appliance/test-appliance-restore-reject.sh` + `tests/stack/appliance/test-appliance-restore-commit.sh` + `tests/stack/appliance/test-appliance-wizard-spool.sh` | 1 ✅ |
...
| Restore leg: the checked-in encrypted v1.20.0 fixture restores to a keep-preserved target; wallet, Tor identity, opaque RPC/onion secrets, and both fixture and target chain sentinels survive without a resync (#2001/#2230). The dashboard password survives in `config.json`, and its archived bcrypt and fingerprint are kept exactly while the fingerprint matches it (#2579), proven by an authenticated request. The installer applies the restore to the kept target's data before its first boot, replacing its superseded config and role markers while the chain trees remain intact. The fixture's `xmrig_proxy.*` settings migrate to `xvb.*` unchanged, drive the rendered XvB settings, and leave no `config.json.bak-1x`; `telegram.control` is dropped | `tests/stack/appliance/test-appliance-restore.sh` (`install_restore_to_target` on a kept target) · battery `--phase install` | 1 ✅ · 4 ✅ |
