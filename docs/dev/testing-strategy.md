...
| LAN-only sources on the `*_lan_access` ports (#2616): the rule admits loopback, private and CGNAT sources only, compose holds a port on `127.0.0.1` when the rule cannot be installed, doctor tells a held port from an exposed one, and every publish of the three ports is an explicit IPv4 bind. Live: a dial from a non-private source is dropped and one from a private source connects | firewall | 1 ✅ (`tests/stack/firewall/lan-guard.sh`) · 4 ▶ (e2e `local-pruned-main-rpclan` row, iptables; KVM provision phase, `tests/os/appliance-lan-guard-leg.sh`, nft) |
...
| The four `CONTROL_NODE_ENDPOINT_KEYS` (#1888/#1924) stay the fixed local placeholder — never move to a `*.remote.*` value — on a non-remote chain; the approval gate's silent-skip of `preflight_remote_nodes` on local/off is safe only because that render-level invariant holds, and it is asserted against the rendered `.env`, with a same-config firing control proving the identical `remote.*` fields DO move the render once the chain actually is remote | config → render | 1 ✅ (`tests/stack/control/test-control-editable-allowlist.sh`) |
| `tari.mode` local / remote / off (#103/#1855/#1916): tari container/onion present/absent, sync gate against a remote target, an `off` machine that runs no Tari and merge-mines nothing once Monero sync releases its separate hold, the retained Tari directory identified independently of normal chain growth, P2Pool's final pre-exec launch record, and both setup wizards writing the key explicitly — including `./pithead setup` declining Tari and still provisioning | config | 1 ✅ (harness logic, `selftest-tari-mode-off.sh`; appliance verdict instruments, `appliance-tari-mode-leg.sh --self-test`; the CLI wizard's question and its disk-derived default, `tests/stack/test-wizard-tari.sh`) · 4 ▶ (live matrix) |
...
| A stop reaches the Tari node (#2627): `tari` runs under an init (compose `init: true`; the quadlet renderer's `RunInit=true`) because the wrapper and upstream's start script both `exec`, which would leave `minotari_node` as PID 1; a PID 1 that reaps nothing can turn zombie and then cannot be signalled | `docker stop` / `podman stop` | 1 ✅ (`standalone/test_compose.sh`; `test-render-quadlet.sh`) · 4 ▶ (compose on the appliance's podman: PID 1 is podman's init, `appliance-tari-mode-leg.sh`; the quadlet units are not deployed yet) |
...
| `upgrade` refuses a Tari major that migrates the node database when the data volume lacks `data.mdb`'s size plus 5 GiB (#2636), before any container is recreated | stubbed `df`/`docker`, sparse `data.mdb` (`tests/stack/lifecycle/tari-upgrade-space.sh`) | 1 ✅ |
| `os-update` and the dashboard's `os-verify`/`os-install` refuse a `data_migration` bundle on the same bound (#2645), through one shared guard: no `rauc install`, no marker, no floor raised, and the dashboard keeps the staged download. Remote or off Tari, no database, and a non-migrating bundle pass; an unreadable `df` warns. Removing either call site fails its door's rows | stubbed `df`/`rauc`, sparse `data.mdb` (`tests/stack/appliance/os-update-migration-space.sh`) | 1 ✅ · 4 ▶ (the provision phase's migration leg fills `/data` to below the bound, asserts the refusal leaves the idle slot and `/data` markers untouched, then frees it and installs) |
...
| A live Tari node stranded from its peers stops reading healthy, and recovers (#2464). An iptables rule in the tari container's own network namespace, tagged `pithead-e2e-fault-tari-stranded`, drops its traffic to tor. It is not a host `DOCKER-USER` rule, because same-bridge traffic does not reach that chain without br_netfilter (job 1315). The leg fails straight away if the rule is not in place, and reports the rule's drop counter whenever a verdict wait fails. The leg asserts the dashboard verdict goes amber within 10 minutes plus a poll, then red with a non-zero doctor exit and a red `pithead status` line within 30 minutes plus a poll. It freezes tari (SIGSTOP, standing in for a migration) and asserts the automatic restart is withheld. It then removes the rule, asserts the restart fires and the verdict returns to green after catch-up. The rule is also removed by an EXIT trap, and the leg asserts none is left | a local Tari node, passwordless `sudo iptables` on the box | 1 ✅ (verdict, guards and alerts: `test_tari_health.py`; doctor/status: `tests/stack/test-tari-chain.sh`) · 4 ▶ (`run.sh --tari-stranded`; bench-ci phase `tari-stranded`, about an hour, opt-in) |
...
| A live Tari node accepts P2Pool 4.18.1's blocks (#2589, V5 of #1129). The shipping `minotari_node` build runs only MainNet and StageNet. Mainnet is off limits, and StageNet never activates the coinbase-prefix payload. The node is therefore Tari's published `-esme` image of the release `docker-compose.yml` pins: the same source revision, built for the testnet target, running LocalNet with the payload active from height 0. The leg prints both images' revisions and build targets, and fails unless the revision and version match the shipping image. The node runs alone on an `--internal` docker network at LocalNet's fixed difficulty of 1. A throwaway P2Pool built from `build/p2pool` with the archive under test joins that network and `mining_net`, reads the box's monerod and mines until it reports blocks at five heights. `tests/integration/mergemine/localnet_probe.py` then reads every reported block back from the node. Tari's SubmitBlock also answers OK for an orphan, so a block counts only when it is on the node's main chain at the reported height, mined with RandomXM, and every submitted height needs one (at least three). Each accepted block's parent must be the previous accepted block, and P2Pool must take the next height's template after it. Nothing reaches Tari mainnet | local Monero on the box, both wallets in `config.json` | 1 ✅ (probe decoding and verdicts, the image pin, skips and cleanup, `selftest-mergemine-localnet.sh`) · 4 ▶ (`run.sh --mergemine-localnet`; bench-ci phase `mergemine-localnet`) |
| Caddy TLS scheme; Tor onion provisioning; HugePages/AVX2; real disk pressure; prune DB size | real host | 4 ▶ |
| HugePages each daemon actually holds (#2685): monerod and p2pool are sampled every 10 s for the whole destructive run from `/proc/<pid>/smaps_rollup` (`Private_Hugetlb` + `Shared_Hugetlb`), never from a `HugePages_Free` delta, which other users of the pool pollute. Red when a running daemon's rollup was never readable, when it held no hugetlb page at any reading, when one instance ran 300 s without one, or when three instances never held one (the restart loop p2pool fell into in #78, before #2562's cap raise made the modern shortfall silent instead); the peaks land in `results/hugepages-peak.json`, the measurement the appliance's reduced tier is sized from (job 1115@701013bb1faaea1a9a337cde16dbe79fbca797d1: p2pool 1305 pages, monerod 129). The row also carries the standing bound the resize is proved by — this run's combined peak + 256 (second-seed caches) `<=` `REDUCED_PAGES`, read from the checked-out branch's own `os/overlay/pithead-hugepages` | live stack, every destructive run | 1 ✅ (tally, gate, artifact, `run.sh` wiring, the pool bound against fixture and real pins, and the `/proc` read against a stand-in process, `selftest-hugepage-probe.sh`) · 4 ✅ (job 1115@701013bb1faaea1a9a337cde16dbe79fbca797d1) |
...
| Worker Inspect edit on `pools` (#1002b): the rig's own reading is credential-stripped and must not be written back (#113), and the dashboard's `last_applied.pools` record is stripped the same way, so the operator-supplied probe is both the value applied and the value restored (#2470) | live rig control API on | 1 ✅ (`selftest-rigforge-writable-keys.sh` — gate, restore source, probe credential check, malformed probe; `selftest-rigforge-pools-settle.sh` — the async settle and when the restore ledger retires the probe, #2407) · 4 ▶ (`run.sh --rigforge-control`, #1002b; operator supplies `IT_RIG_POOLS_PROBE`) |
| `autotune` and `watchdog` writable-key edits | live rig control API on | **Permanently not covered, on purpose** — `autotune` starts a real tuning run; `watchdog` removes thermal protection from a rig at its temperature ceiling. Both are refused on borrowed hardware; each refusal is a `[by-design]` leg row and is asserted at tier 1 (`selftest-rigforge-writable-keys.sh`) so it cannot be undone silently. |
...
| Control-result retention (#1990): after this phase's own burst of real control requests (worker-apply, pools, reverse, rollback, upgrade), the host-side `control_prune_results` has kept `results/` within its documented count and byte caps, and the stack is still healthy — the release-gate proof that pruning runs on a real drain without breaking anything it touches (the caps themselves, including the protected-file guarantees, are driven directly against synthetic excess at tier 1) | `control_prune_results` + a real drain | 1 ✅ (`tests/stack/control/control-results-prune.sh` — excess results/archives, count/byte caps, protected files) · 4 ▶ (`run.sh --rigforge-control`, end of phase) |
...
| Restore-at-setup (#909): archive+passphrase spool contract (accept, bad passphrase, oversize, malformed → fallback) — `dashboard/tests/web/test_wizard_handoff_rig_and_restore.py` and `dashboard/tests/frontend/wizard/wizard-submit.test.mjs` for the server+client contract, `tests/stack/appliance/test-appliance-restore.sh` for `firstboot_consume_restore` against a genuine backup archive, including a target-side chain-data collision the merge must resolve in the target's favor (#2195); a failure injected inside the commit (the dashboard swap, the chain merge, the rollback itself) puts back every item already replaced and removes the names the merge added (#2689) | `dashboard/tests/web/test_wizard_handoff_rig_and_restore.py` + `dashboard/tests/web/test_wizard_restore_limits.py` + `dashboard/tests/frontend/wizard/wizard-submit.test.mjs` + `tests/stack/appliance/test-appliance-restore.sh` + `tests/stack/appliance/test-appliance-restore-commit.sh` | 1 ✅ |
| Image invariants: variant stamp, enabled units, watchdog/governor config, unwanted-package and unsupported-test-tree absence, dev-keyring refusal | `tests/os/verify-image.sh` (static, no KVM) | build-time ✅ |
...
| Dashboard exposure boundary (#1021/#1049/#2070/#2280): the rendered Caddyfile publishes the LAN v4 and the ULA but never the globally-routable v6; the bind line excludes it; a pinned `dashboard.host` does not reopen it AND still binds the box's own LAN address (#1089: pinned to a NAME, the documented appliance case — a pinned IP literal put the box's own address into the site list too, so a bind derived from `$site_hosts` and one derived from `hostname -I` rendered identically and the mutation stayed green); a box with only a public address still binds loopback; EVERY site block carries a bind (the count invariant that caught the unbound HTTPS onion vhost); the onion vhosts bind the container-bridge gateway and not a wildcard. The provision battery adds an unrouted RFC 3849 address and a ULA to the guest LAN interface before the wizard submit, then requires the pinned site, LAN v4 and ULA binds/listeners, no wildcard/public bind or live listener, a refused curl to the global address and doctor's dashboard-listener OK — repeated with the onion-enabled appliance configuration so its vhosts are proven not to reopen the listener; the independent stratum public-IP WARN remains truthful, and the tier-1 public-bind mutation proves that the live row can fail | `tests/stack/dashboard/test-dashboard.sh` (sourced `generate_caddyfile`, stubbed `hostname`) · `tests/os/appliance-dashboard-exposure-leg.sh` · battery `--phase provision` | 1 ✅ · 4 (added — unverified until the next provision battery) |
...
| A chain service that dies AFTER a migrating slot commits (#2588, V10 of #1129): the release `up` succeeded and `pithead-boot` exited 0, so only the operator's surfaces can report it. On the migration leg's committed guest, with the pending marker consumed, Tari is stopped by its compose label; `pithead status` must exit non-zero with a ✗ (or ⚠ running but UNHEALTHY) row for it (the hold's own ⚠ row does not count), `pithead doctor --json` must FAIL the revenue check on it with no migration-pending note, and `/api/state` must carry the `Tari DOWN` badge. `./pithead up`, the recovery the boot path's FAULT line names, must bring all three back: status shows it running, doctor no longer fails on it, and the badge clears on a dashboard that kept its start time (a restarted dashboard has no badge to clear). The badge fires only for a node the dashboard has reached, so the leg first waits until the dashboard has reached the released node: both containers up for 180 s (several times the usual gap between two dashboard Tari polls; a cycle slow enough to leave the window without one can only turn the badge row red) with no Tari gRPC error and no failed data-collection cycle in its log, and an unreadable log counts as not reached | `tests/os/appliance-chain-fault-leg.sh --self-test` (verdicts, the dashboard precondition and its probe's remote command, the leg against a stubbed guest with one red scenario per surface and phase, its call site), driven from `tests/os/selftest-row-payloads.sh` | 1 ✅ · 4 ▶ (battery `--phase provision`, the migration leg) |
| Install-to-disk copies a COMPLETE system; reinstall keep/fresh paths | battery `--phase install` | 4 ✅ |
| RC1 reinstall and setup recovery (#1954/#1955/#1966): a 1.x `xmrig_proxy` pre-fill reaches the current page as `xvb` with a value-free migration notice; node preflight refusal retains the safe answers; then a separate post-marker setup fault reaches the named failed page and authenticated retry reopens the same answers for correction. The fault retags only the dashboard service's image, NOT the #2050 whole-file stub: that stub broke every `docker compose` call including `provision_tor`'s tor-only one, so it faulted three lines before `setup` ever set `DEPLOYMENT_COMPLETED=true` — the marker was never on, so #2054's clear of it went unexercised at tier 4 despite the row below reading ✅ (#2061). Retargeting the dashboard image alone lets `provision_tor` and both `render_env` calls succeed and fails only in `stack_up`, well past the marker. The machine it hands back must also be re-provisionable — `wizard_keep_failed_config` clears `DEPLOYMENT_COMPLETED`, which `setup` sets a third of the way in and whose survival made every retry refuse as already provisioned; the retry that follows in the same phase (a fresh browser submit through to "stack containers are running") is what actually discriminates a cleared marker from an uncleared one | `tests/stack/appliance/test-appliance-setup.sh` (the marker clear and its no-.env half) · battery `--phase install` pre-fill + `--phase provision` preflight and failed-attempt legs | 1 ✅ (marker clear, controlled against a build with only that block removed) · 4 ✅ (re-armed for #2061 and measured on bench-ci job 224: `DEPLOYMENT_COMPLETED` observed `true` before the fault and `false` at the exact moment the failure was recorded — job 187 had only shown a failure was recorded, which reads the same whether the fault fires before or after the marker, so this second assertion is what actually discriminates a regression to the #2050 shape from a real post-marker failure) |
| Post-provision control on the appliance (#1966): a benign energy edit applies; a disruptive operational edit is refused without typed `APPLY`, applies with it, and is restored. Doctor returns all `{status,message}` rows, including a failed result carrying the complete document when its own exit is nonzero with monerod deliberately stopped. A p2pool tail is clamped to 200 lines/65,536 bytes, records an applied result when the byte cap closes its pipe, and redacts the known payout address; wallet-rpc logs remain refused | `tests/os/appliance-diagnostics-leg.sh` + battery `--phase provision` | 1 ✅ (verdict failure controls plus control/diagnostics route and host-runner suites) · 4 (added — unverified until the combined battery) |
...
| Restore leg: the checked-in encrypted v1.20.0 fixture restores to a keep-preserved target; wallet, Tor identity, opaque RPC/onion secrets, and both fixture and target chain sentinels survive without a resync (#2001/#2230). The dashboard password survives in `config.json`, and its archived bcrypt and fingerprint are kept exactly while the fingerprint matches it (#2579), proven by an authenticated request. A carried archive also clears only the target's superseded config and role markers, so firstboot consumes it while the chain trees remain intact. The fixture's `xmrig_proxy.*` settings migrate to `xvb.*` unchanged, drive the rendered XvB settings, and leave no `config.json.bak-1x`; `telegram.control` is dropped | `tests/stack/appliance/test-appliance-install-restore.sh` (real installer block) · battery `--phase install` | 1 ✅ · 4 ✅ |
| Wizard's real HTTP flow provisions the STACK (images verified, direct clearnet egress dropped and Tor-routed egress succeeds from the Monero container through its shipped `/usr/bin/curl`, a direct IPv6 dial dropped whenever that container holds a global v6 address, as read from `/proc/net/if_inet6` with the image's `/usr/bin/cat` since the image ships no `ip`, miner up); a direct dial counts as dropped only when it times out, and a probe that cannot run (curl, the v6 guard's cat, or a dial that fails any other way) is a counted unverified failure, never a pass; unaided reboot return; after each of M10's three live-stack power cuts every pre-cut container, stored image digest, a readable monerod height at or above the one flushed before the cut, miner, Caddy, and committed slot return; commit-gate honesty both ways; the migration hold starts the chain only post-commit | `tests/os/appliance-egress-leg.sh --self-test` (stubbed guest: missing curl or cat, each direct-dial exit, a failed Tor dial, v6 scope) · battery `--phase provision` | 1 ✅ · 4 ✅ (the changed egress probes, bench-ci#532, passed in the 2026-09-24 provision run) |
...
| Doctor's HugePages verdict (#2610): a pool short of this machine's budget (`hugepages_decision_pages`: 3072, or the appliance's recorded reduced pool; never below P2Pool's 1296 pages of dataset plus two caches) WARNs, never FAILs, naming the pool against that target in pages and the shortfall in MiB; below P2Pool's 1040-page dataset, a zero pool included, it names the up to 2592 MiB of ordinary RAM P2Pool falls back to under its 4 GiB limit, pinned at 1039/1040, with that limit read off the Compose file and the Quadlet renderer; from 1040 up to the budget it names the fallback as conditional on monerod's own RandomX pages leaving P2Pool its 1040, pinned at 1040, 1296 and 3071; the 1296 floor is pinned at 1295/1296 on the released tier and labelled in the message; a non-numeric total reads as could-not-read; the host fix names the full GRUB boot parameters, `update-grub` and a reboot; the reduced tier is held to its recorded pool; the appliance wording passes the `_dr_leaks` sweep with a host-wording control; the full budget reads OK; `pithead doctor` itself carries the verdict | `tests/stack/doctor/test-doctor-memory.sh` (sourced, fixture meminfo via `PITHEAD_MEMINFO`) | 1 ✅ |
...
| 1 — shell suite (`tests/stack/`) | **Linux, GNU coreutils, non-root**, and a longer tool list than it looks: `bash`, `jq` **1.7 or newer**, `e2fsprogs`, a system `python3` on PATH, `xxd`, `sudo`, and a **UTF-8 locale**. Refuses on Darwin (#2041); root bypasses the permission-denied fixtures. Missing system Python, xxd, or a UTF-8 locale fails *quietly* — see the note below. | Yes — the container is how a non-Linux host runs this at all |
...
A missing tool in this suite does not announce itself. Measured while building the image
(#2078), each of these produced a **wrong verdict rather than an error**, which is the exact
failure class the image exists to remove:
...
| Compose interpolation, **security/hardening** invariants, and generated Caddyfile parsing | 1 | every PR | ✅ required |
