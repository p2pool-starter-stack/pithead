...
| Secure Boot (#2055 G2): a second guest with `firmware.feature0.enabled=yes` measures whether the image reaches userspace under Secure Boot, instead of leaving the flag (every OTHER guest in the battery pins `enabled=no`) an unread claim. `shim-signed` is the only signed link in the image's boot chain — `grub-efi-amd64` and the kernel ship unsigned, with no sbsign/mokutil/enrolled-key tooling in the repo — so the defined-but-not-booted guest records the known deferred state without failing unrelated KVM evidence. A guest that cannot be defined remains unmeasured and fails. #2187 defers signing past 2.0.0 to `v2.x - post-GA`; the same check reports signing works when that work lands | `tests/os/secure-boot-boot-verdict.sh` · battery `--phase boot` | 1 ✅ (fixture, `tests/stack/appliance/test-appliance-boot-verdicts.sh`) · 4 (added — unverified until the next battery run) |
