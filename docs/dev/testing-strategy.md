...
| `os-update` and the dashboard's `os-verify`/`os-install` refuse a `data_migration` bundle on the same bound (#2645), through one shared guard: no `rauc install`, no marker, no floor raised, and the dashboard keeps the staged download. Remote or off Tari, no database, and a non-migrating bundle pass; an unreadable `df` warns. Removing either call site fails its door's rows | stubbed `df`/`rauc`, sparse `data.mdb` (`tests/stack/appliance/test-appliance-os-update-migration-space.sh`) | 1 ✅ · 4 ▶ (the provision phase's migration leg fills `/data` to below the bound, asserts the refusal leaves the idle slot and `/data` markers untouched, then frees it and installs) |
