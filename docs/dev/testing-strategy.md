...
| LAN-only sources on the `*_lan_access` ports (#2616): the rule admits loopback, private and CGNAT sources only, compose holds a port on `127.0.0.1` when the rule cannot be installed, doctor tells a held port from an exposed one, and every publish of the three ports is an explicit IPv4 bind. A DIY Docker host gets `pithead-lan-guard.service`, which restores the rule before `docker.service` after a reboot (#2749). Live: a dial from a non-private source is dropped and one from a private source connects, again after the rule is stripped and the boot unit runs | firewall | 1 ✅ (`tests/stack/firewall/lan-guard.sh`) · 4 ▶ (e2e `local-pruned-main-rpclan` row, iptables; KVM provision phase, `tests/os/appliance-lan-guard-leg.sh`, nft) |
