...
| Failing appliance rows carry their evidence, not just a verdict (#2060): the hostname rows print all six identity fields want-beside-got plus what Avahi published and on which interface (the address moved between runs, the interface is the discriminator); the control-runner rows print the result the runner returned and the value read back; the doctor row stays total over a FLATTENED `checks`, the shape it is named after; the bundle rows print the tail of the build log, with a missing log, an empty log and a failed build each getting their own sentence | `tests/os/appliance-hostname-leg.sh`, `appliance-diagnostics-leg.sh`, `provision-browser-submit.sh` and `appliance-approval-verdict.sh` `--self-test`, plus `tests/os/selftest-row-payloads.sh` and `tests/os/bundle-build-evidence.sh --self-test` | 1 ✅ (each payload driven against the shapes the verdict refuses, and a mutation of each field reddens its own control) · 4 (the rows themselves stay red until the defects behind them are fixed — this is the evidence step the issue asks for first) |
...
| Restore leg: the signed-v1.20.0 encrypted fixture is uploaded instead of the form onto an existing appliance disk. The restored running stack must carry its wallet, Tor identity and RPC/dashboard/onion-secret fingerprints, while both the fixture's chain sentinel and the target's pre-restore sentinel survive. The closing verdict (#1091) requires the stack to actually come back up (`podman ps`) AND the restored `--wallet` rendered into the p2pool container to match: `config.json` landing on disk alone proves only that the archive was unpacked, not that anything is RUNNING it. The container command line is used rather than p2pool's stratum stats because those require a synced monerod. | `tests/os/restore-live-state-verdict.sh`, `tests/stack/appliance/test-appliance-identity-boot.sh` (fixture `podman ps` / wallet strings), `tests/os/fixtures/v1.20.0/` · battery `--phase install` | 1 ✅ · 4 (added — pending focused KVM run) |
