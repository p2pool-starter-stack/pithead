...
| `backup`/`restore` | real box | 1 ✅ (partial) · 4 ✅ (`--lifecycle`/`--safety-backup`) |
| `reset-dashboard` (#2346) | real box | 1 ✅ (partial) · 4 ✅ (`--reset-dashboard`) |
...
- CLI breadth in automation. ✅ `backup`/`restore` are exercised end-to-end: by
  ...
  combined hardware run remains pending. `reset-dashboard` (#2346) is exercised live by
  `--reset-dashboard`: the wiped dirs come from `.env`, not an unapplied `config.json` (#139), the
  recreated dirs are owned by `APP_UID` (#550), the chains never rewind, dashboard/p2pool come back
  healthy, and a forced real compose failure reaches the friendly retry branch (#557).
