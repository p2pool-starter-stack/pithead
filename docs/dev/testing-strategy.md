...
| Doctor's HugePages verdict (#2610): a pool short of this machine's budget (`hugepages_decision_pages`: 3072, or the appliance's recorded reduced pool; never below P2Pool's 1296 pages of dataset plus two caches) WARNs, never FAILs, with the shortfall in pages and MiB; below P2Pool's 1040-page dataset, a zero pool included, it names P2Pool's crash-loop under its 1 GiB limit, pinned at 1039/1040; the 1296 floor is pinned at 1295/1296 on the released tier; the host fix names the full GRUB boot parameters, `update-grub` and a reboot; the reduced tier is held to its recorded pool; the appliance wording passes the `_dr_leaks` sweep with a host-wording control; the full budget reads OK; `pithead doctor` itself carries the verdict | `tests/stack/doctor/test-doctor-memory.sh` (sourced, fixture meminfo via `PITHEAD_MEMINFO`) | 1 ✅ |
