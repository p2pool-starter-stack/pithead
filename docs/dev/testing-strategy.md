...
| Wizard's real HTTP flow provisions the STACK (images verified, Tor-only egress enforced, miner up); unaided reboot return; after each of M10's three live-stack power cuts every pre-cut container, stored image digest, a readable monerod height at or above the one flushed before the cut, miner, Caddy, and committed slot return; commit-gate honesty both ways; the migration hold starts the chain only post-commit | battery `--phase provision` | 4 ✅ |
