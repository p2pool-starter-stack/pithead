...
| A chain service that dies AFTER a migrating slot commits (#2588, V10 of #1129): the release `up` succeeded and `pithead-boot` exited 0, so only the operator's surfaces can report it. On the migration leg's committed guest, with the pending marker consumed, Tari is stopped by its compose label; `pithead status` must exit non-zero with a ✗ row for it (the hold's own ⚠ row does not count), `pithead doctor --json` must FAIL the revenue check with no migration-pending note, and `/api/state` must carry the `Tari DOWN` badge with Tari not connected. `./pithead up`, the recovery the boot path's FAULT line names, must bring all three back. The badge fires only for a node the dashboard has reached, so the leg first waits until the dashboard has read the released node for two polls | `tests/os/appliance-chain-fault-leg.sh --self-test` (verdicts, the dashboard precondition, the leg against a stubbed guest, its call site), driven from `tests/os/selftest-row-payloads.sh` | 1 ✅ · 4 ▶ (battery `--phase provision`, the migration leg) |
