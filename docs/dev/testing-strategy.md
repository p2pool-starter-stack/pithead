...
| `tari.mode` local / remote / off (#103/#1855/#1916): tari container/onion present/absent, sync gate against a remote target, an `off` machine that runs no Tari and merge-mines nothing while still mining Monero, the retained Tari directory identified independently of normal chain growth, P2Pool's executed argv read from PID 1, and both setup wizards writing the key explicitly — including `./pithead setup` declining Tari and still provisioning | config | 1 ✅ (harness logic, `selftest-tari-mode-off.sh`; appliance verdict instruments, `appliance-tari-mode-leg.sh --self-test`; the CLI wizard's question and its disk-derived default, `tests/stack/test-wizard-tari.sh`) · 4 ▶ (live matrix) |
