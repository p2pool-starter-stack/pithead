...
| Rig role: no containers, mines from the baked binary, takes an A/B update like a coordinator; after reboot its `pithead-boot` transaction settles active before the rig result is read | battery `--phase rig` | 1 ✅ (settled-unit wait fixture) · 4 ✅ |
