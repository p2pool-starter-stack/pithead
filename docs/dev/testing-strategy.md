...
| `rotate-secrets` (#2344): the recreate (not restart) actually lands — monerod RPC and the xmrig-proxy control API accept the new credentials and refuse the old ones, the live container argv carries the new stratum access-password, and the pre-rotation `.bak-<stamp>` copies are owner-only and swept by the leg (nothing else sweeps them) | real box | 1 ✅ (preview text, mutation-lock placement) · 4 ▶ (`--rotate-secrets`, requires `--safety-backup`) |
