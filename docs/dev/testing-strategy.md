...
| Wizard's real HTTP flow provisions the STACK (images verified, direct clearnet egress refused and Tor-routed egress succeeds from the Monero container through its shipped `/usr/bin/curl`, miner up); a missing probe is a counted unverified failure; unaided reboot return; after each of M10's three live-stack power cuts every pre-cut container, stored image digest, non-regressing monerod height, miner, Caddy, and committed slot return; commit-gate honesty both ways; the migration hold starts the chain only post-commit | battery `--phase provision` | 4 ✅ |
