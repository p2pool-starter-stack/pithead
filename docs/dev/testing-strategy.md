...
| `monero.view_key` / `tari.view_key` (#381/#462): payout-confirmation wallet-rpc/tari-wallet wiring | config → fake wallets → persisted state + configured webhook | 3 ✅ (`mini-stack`: one confirmed payout reaches `/api/state` and one alert; replay, empty-wallet, and disabled/no-dial controls for both chains, #2267) · 1 ✅ (the Tari leg's logic, `selftest-tari-wallet.sh`) · 4 ▶ (needs `IT_MONERO_VIEW_KEY` or `IT_TARI_VIEW_KEY`+`IT_TARI_SPEND_PUBLIC_KEY`; a new Monero payout landing is real-money real-time, not e2e-reachable; the Tari wallet backfills its known past payouts from a set birthday, through the local node only, #2731) |
