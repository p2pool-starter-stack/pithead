...
| The four endpoint members of `CONTROL_NODE_PREFLIGHT_KEYS` (#1888/#1924) stay fixed local placeholders on a non-remote chain; the two login members preserve their masked partner and the dashboard auth render during standalone local edits. The same preflight set forces endpoint and login changes through the staged authenticated probe in remote mode | config → render/control | 1 ✅ (`tests/stack/control/test-control-editable-allowlist.sh`, `dashboard/tests/service/test_env_key_perimeter.py`) |
...
| RC2 sensitive-commit/node rows (#1959/#1898/#1943/#1946, retargeted by #2076): a sensitive day-two commit is refused without the typed confirmation envelope and leaves live identity unchanged; a confirmed one applies, audits against the signed-in actor with **no** `approver` field, and converges hostname/mDNS/certificate identity; a physical-presence-only dashboard password stays refused. A standalone local Monero node username edit, then a password edit, each previews as a `CONFIRM` change, applies with typed `APPLY`, and must leave monerod's and the dashboard's container environment and p2pool's `--rpc-login` argument carrying the new login (checked against podman's inspect output, where names carry no Docker-style leading slash). The fixture sets `p2pool.clearnet` host-side first (#165; on an appliance it is fixed at setup and never dashboard-committable), so the dashboard proposal changes only `monero.*` and `tari.*`. Reserved reachable Monero/Tari endpoints must pass the host preflight, land in rendered runtime state and the current p2pool container's narrowly extracted arguments, and make an endpoint-bound current-startup `uses chain_id` consumer probe pass before the original local-node config is restored; the restore also runs when the leg fails early. The guest-side fake Telegram transport is gone with the approval round-trip, so this leg now drives the ordinary authenticated control route | `tests/os/appliance-config-approval-leg.sh`, `tests/os/appliance-node-runtime-leg.sh` plus the existing `tests/integration/lib/mergemine-probe.sh` verdict | 1 ✅ (host policy suite in `tests/stack/test-confirm-approval.sh`; pure round-trip controls) · 4 (written — requires reserved node inputs and remains unverified until the combined battery) |
