...
| `--workers <n>` | Miners expected online while mining (default `2`). In `e2e.sh`, `3` also starts one temporary one-thread XMRig clone on the borrowed rig. The leg discovers the online baseline labels, requires exactly that baseline plus the clone, observes xmrig-proxy stop and recover with every label online, requires finite non-negative worker rates with a positive aggregate and advancing accepted shares (including the clone), and writes aggregate rate, accepted shares, whether a process sample was captured, peak clone CPU/RSS, and dashboard response latency to the run log and `multi-worker-metrics.json`. These measurements do not carry ceilings. The clone is stopped and its process identity proved before the borrowed rig is restored. |
