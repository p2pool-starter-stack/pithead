...
| `--workers <n>` | Miners expected online while mining (default `2`). In `e2e.sh`, `3` also starts one temporary one-thread XMRig clone on the borrowed rig. The leg discovers the baseline worker labels and requires that baseline plus the clone after lifecycle recovery; it does not assume a fixed bench fleet size. The clone stops before the borrowed rig is restored. |
