...
| `--lifecycle` | Also run the lifecycle phase (restart, apply secret-preservation, then backup→restore). Restore commands, health, and restored pool/secrets must all succeed before later fault injection can run. |
