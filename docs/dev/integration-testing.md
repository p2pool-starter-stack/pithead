...
| `--workers <n>` | Miners expected online while mining (default `2`). In `e2e.sh`, `3` also starts one temporary one-thread XMRig clone on the borrowed rig. The leg requires the requested number of distinct online labels including the clone and the borrowed miner's configured label, observes xmrig-proxy stop and recover at that capacity, requires an observed finite non-negative worker-rate set with a positive aggregate and advancing accepted shares, and writes the peak observed aggregate rate and counters, whether a process sample was captured, peak clone CPU/RSS, and dashboard response latency to the run log and `multi-worker-metrics.json`. These measurements do not carry ceilings. Evidence is sampled while the harness runs because its final restore can intentionally leave no workers online. A RigForge control apply can replace the borrowed worker label, so the gate re-reads it after the re-arm; unrelated bench workers can join or leave. The clone is stopped and its process identity proved before the borrowed rig is restored. |
