...
rig's control API for #516's rig-side edit. Each restore names its key and rig on stderr, not its
value. The value reaches `jq` on stdin and is never passed as a command-line argument, because the
pools original carries the stratum `pass` and any local user can read a process's arguments
