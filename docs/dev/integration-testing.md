...
| `--rotate-onion` | Also run `rotate-dashboard-onion` on a reserved, non-production fixture ([#2345](https://github.com/p2pool-starter-stack/pithead/issues/2345)): mint a fresh `.onion` address and client-auth key, then prove the new address answers over the real Tor network, the retired address becomes unreachable with its original key after a bounded transition, the Caddyfile names only the new vhost ([#546](https://github.com/p2pool-starter-stack/pithead/issues/546)'s regression), and `HOST_IP` survives the render ([#356](https://github.com/p2pool-starter-stack/pithead/issues/356)'s regression). Destructive-then-restored: keeps the client credential in an owner-only file on the box, swaps the saved Tor hidden-service key files back into place, verifies their fingerprints and hostname, renders the old address into the derived files, and proves it externally reachable again; the run-level EXIT restore does the same after a failed or interrupted phase. Because this is an explicitly selected proof, a missing, invalid, or unreachable fixture fails instead of skipping. |
