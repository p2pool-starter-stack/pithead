...
| `--lifecycle` | Also run the lifecycle phase (restart, apply secret-preservation, then backup→restore). Restore command or health failure, and any failed restored pool/secrets assertion, prevents later fault injection; peer-timing pool warnings remain non-fatal. |
