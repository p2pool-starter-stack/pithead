...
| the restore leg | `tests/os/run.sh --phase install` | a checked-in encrypted v1.20.0 fixture generated from the signed compose bundle, uploaded through `/submit-restore` instead of the form onto an existing appliance disk — the running wallet, Tor identity and opaque RPC/onion secrets must match the prior-release fixture, the dashboard hash and fingerprint must be regenerated from the preserved password and authenticate with it, both its chain sentinel and the target's pre-restore sentinel must survive, and its removed 1.x keys (`xmrig_proxy.*`, `telegram.control`) must migrate or drop as documented, with the pre-migration copy kept in `config.json.bak-1x` |
