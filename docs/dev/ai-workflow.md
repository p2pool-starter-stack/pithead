...
| Review correctness, security, and acceptance | A fresh non-author session; prefer the other provider when available | PASS or RETURN at the full current SHA, with commands and limits, recorded as the `adversarial-review` commit status |
...
Use `state=failure` for a RETURN. Merge explicitly with `gh pr merge <n> --merge`
once the required checks and the review status are green. Do not use `--admin`,
and do not enable auto-merge: tier 4 runs on the bench through the private
bench-ci runner, on an exact commit, and reports to the PR rather than to GitHub
checks. When a change needs that tier, record the job's tier, SHA, and result in
