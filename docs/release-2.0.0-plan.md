...
| P4 | A KVM bench advertises `image_upgrade_ready=true`, else `phases=all` cannot run `image-upgrade` | bench-ci docs/tiers.md:168-170 [r]; R2 | none does [r report §2]; bench-ci#560 sits in `maintenance & gates` (frozen) [r `gh`] | owner (hardware, milestone) |
...
|---|---|---|---|---|
| 1 | tier4-kvm `phases: ["all"]` on the frozen SHA; job id on #2602 | every phase green: boot, update, install, provision, rig, media, fault, reset, image-upgrade [r bench-ci docs/tiers.md:164-166] | 9 × 30–90 min = 4.5–13.5 h, solo tier [r tiers.md:118]; default timeout 150 min → pass `timeout_minutes` [i] | P4; #1651 |
...
| KVM benches | `tier4-kvm` is `solo` (one job per bench, free-memory floor at boot); each bench now pushes to its own registry namespace, ending the cosign race that killed five KVM jobs today (930, 932, 934, 938, 874). | [r docs/tiers.md:351-357; bench-ci#568 → PR #593 merged 23:16Z] |
...
| Node guard | KVM and matrix-e2e hold read leases on the shared nodes; node-serving e2e and `baseline-refresh` need the write; writer preference. A failed restore keeps its lease until an operator posts `POST /node-guard/release`; the refusal names holder, mode, age; the status page shows `OPERATOR ACTION` after 30 min (a 3.5 h stall today read `ok` before). | [r docs/tiers.md:38-49; bench-ci#576 → PR #577; bench-ci#586 → PR #590 merged 22:36Z] |
| Canary as node provider | `@nodes` resolves at dispatch to the node-serving bench; a credential fingerprint refuses a mismatched dialer; a node > 50 blocks off the explorer tip does not serve. | [r docs/operations.md:337-345, 370-380; docs/tiers.md:440-450; bench-ci#557] |
| Migration fence | A head pinning another Tari/Monero major than the bench's live checkout is refused unless `options.migrates_chain_data`; no routing to a matching peer. | [r docs/tiers.md:96; bench-ci#559 closed, #562 open] |
| HugePages | 3072 × 2 MiB reserved at boot; `tier4-e2e` refuses a deploying job under 1296 free (a 186-page pool OOM-looped P2Pool today). | [r docs/setup.md:20-26; docs/tiers.md:117; bench-ci#569 → PR #574] |
| `tari-observe` tier | Isolated Tari+Tor for a `window_hours` observation against a baseline job's OFFLINE rate; merged, **not applied on the primary** → PR #2611 gets 422. | [r docs/tiers.md:97-98, 120, 212-250; PR #582; bench-ci#584 open] |
