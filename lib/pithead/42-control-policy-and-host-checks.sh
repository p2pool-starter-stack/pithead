...
# (47-/48-os-update-*.sh), the two plain power verbs `sys-reboot`/`sys-poweroff` (#2384,
# 46b-control-power.sh — no update need be pending; `sys-poweroff` does not come back on its own),
# and the two read-only diagnostics verbs `diag-doctor`/`diag-logs`
