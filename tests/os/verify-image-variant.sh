...
if [ "$MODE" = "--test" ]; then
    ...
    # #1892/#1931: built against a non-default registry, every pithead service must carry it AND podman must trust it — one without the other is a first boot, dashboard action or provision that pulls from the wrong registry.
    if [ -n "${PITHEAD_REGISTRY:-}" ] && [ "$PITHEAD_REGISTRY" != ghcr.io/p2pool-starter-stack ]; then
        chk "boot, setup and dashboard-control units pinned to the test registry" '[ "$(grep -lxF "Environment=PITHEAD_REGISTRY=$PITHEAD_REGISTRY" "$ROOT"/etc/systemd/system/{pithead-boot,pithead-firstboot,pithead-setup-again,pithead-control}.service.d/pithead-test-registry.conf 2>/dev/null | wc -l)" = 4 ]'
