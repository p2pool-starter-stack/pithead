...
_rigmedia_stage_image() { # <image> <scratch-disk>
    cp "$1" "$2" || {
        bad "could not stage the removable-media image"
        return 1
    }
}
...
_rigmedia_before_hash_or_cleanup() { # <target>
    RIGMEDIA_HASH=$(_rigmedia_hash "$1") || {
...
_rigmedia_after_hash_or_cleanup() { # <target>
    RIGMEDIA_HASH=$(_rigmedia_hash "$1") || {
...
_rigmedia_containers_or_cleanup() { # <target>
    RIGMEDIA_CONTAINERS=$(_rigmedia_containers) || {
...
_rigmedia_journal_or_cleanup() { # <target>
    RIGMEDIA_JOURNAL=$(_rigmedia_journal) || {
...
_rigmedia_quiesce_or_cleanup() { # <target>
    _rigmedia_quiesce || {
        _rigmedia_fail_cleanup "$1"
        return 1
    }
...
phase_rigmedia() {
    ...
    local img target_disk="/srv/code/bench-vm/pithead-rigmedia-target.img" token jar body scode card empty_before empty_after journal
    ...
    rm -f "$target_disk"
    _rigmedia_stage_image "$img" "$DISK" || return
    ...
    _rigmedia_before_hash_or_cleanup "$target_disk" || return
    empty_before=$RIGMEDIA_HASH
    : >"$SERIAL"
    kvm_preflight || {
        _rigmedia_fail_cleanup "$target_disk"
        return
    ...
    local names
    _rigmedia_containers_or_cleanup "$target_disk" || return
    names=$RIGMEDIA_CONTAINERS
    if [ -z "${names// /}" ]; then
        ok "no compose containers remain after stick-run rig handoff"
    else
        bad "a stick-run rig started containers: '$names'"
    fi
    _rigmedia_journal_or_cleanup "$target_disk" || return
    journal=$RIGMEDIA_JOURNAL
    [ "$journal" != "0" ] &&
    ...
    _rigmedia_after_hash_or_cleanup "$target_disk" || return
    empty_after=$RIGMEDIA_HASH
