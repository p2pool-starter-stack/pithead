...
cmp -s "$td/image.pub" <(tar -xOf "$td/candidate.tar.gz" pithead/cosign.pub) || {
...
failure_out="$(FAIL_IMAGE_PULL=1 PATH="$td/bin:$PATH" PITHEAD_REGISTRY=registry.test \
    ...
    0123456789abcdef0123456789abcdef01234567 "$td/key" "$td/image.pub" 2>&1)" || failure_rc=$?
[ "$failure_rc" -ne 0 ] || {
...
repack_out="$(FAIL_REPACK=1 REAL_TAR="$REAL_TAR" TAR_CALL_COUNT="$TAR_CALL_COUNT" PATH="$td/bin:$PATH" PITHEAD_REGISTRY=registry.test \
    ...
    0123456789abcdef0123456789abcdef01234567 "$td/key" "$td/image.pub" 2>&1)" || repack_rc=$?
[ "$repack_rc" -eq 25 ] &&
