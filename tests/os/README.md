...
pushing a login change through the dashboard is refused outright, then proposes and commits the
endpoint move alone through the dashboard, and only after that lands the login with a direct
host-side config edit (the route "Set up again" names in the dashboard's refusal text) — by then
`monero.mode` is already `remote` and the local monerod/wallet-rpc containers the login has no
business reaching are already retired. The row requires the host preflight and typed
