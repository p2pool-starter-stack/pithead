...
- **provision** — submit a config through the wizard's real HTTP flow and require the STACK to
  ...
  release carries. Its copied build tree names its compose file explicitly and records the file
  hash in `COMPOSE_SOURCE`, so the synthetic build does not need a git origin. The resulting slot
  cannot bring the stack up and falls back uncommitted: the previous slot's boot must put the
  `/data` floor back from the record the raise left, and the same fall-back with the record deleted
  must leave the floor alone and make `os-update` refuse with the failed-update premise.
