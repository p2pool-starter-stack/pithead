...
# #2094's drain, driven against the three spool readings that matter. An empty spool returns at
# once; a spool that never empties must spend its deadline and FAIL rather than fall through to an
...
# #2384: the power leg must be SOURCED by the runner and CALLED by the provision phase — a leg
# that is neither is a file nobody runs — and its own verdict helper is driven here rather than as
# a second tier-1 row, because tests/stack/test-harness-tooling.sh sits at its recorded line
# ceiling and this script is already asserted from there.
grep -Fq '. "$SCRIPT_DIR/appliance-power-leg.sh"' "$HERE/run.sh" || exit 1
grep -Fq 'phase_provision_power_regressions' "$HERE/provision-browser-submit.sh" || exit 1
bash "$HERE/appliance-power-leg.sh" --self-test >/dev/null 2>&1 || exit 1
