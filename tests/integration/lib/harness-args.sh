...
validate_harness_args() { # reads HARNESS_ARGS[]; sets HARNESS_PHASE_ARGS
    ...
    while [ "$i" -lt "${#HARNESS_ARGS[@]}" ]; do
        arg="${HARNESS_ARGS[$i]}"
        case "$arg" in
        --lifecycle | --fault-injection | --auth-fail-closed | --hardening | --subnet | --safety-backup | --rigforge | --rigforge-control | --xvb-routing-smoke | --alert-egress | --mergemine-submit)
        ...
        --scenario)
            ...
            ;;
        *)
            die "--harness-arg does not accept '$arg' — allowed: --lifecycle, --fault-injection, --auth-fail-closed, --hardening, --subnet, --safety-backup, --rigforge, --rigforge-control, --xvb-routing-smoke, --alert-egress, --mergemine-submit, --scenario <name>."
            ;;
        esac
    done
}
