...
run_xvb_routing_smoke() {
    ...
    if [ "$IT_FAIL" -gt "$fails_before" ] || [ -z "$_XVB_BASELINE_ROUTE" ] || [ "$_XVB_EXPECTED_WORKERS" -le 0 ] 2>/dev/null ||
        [ "$(jq_get "$(api_state)" '.shares_window.count')" -le 0 ] 2>/dev/null; then
        it_fail "XvB routing transition has a healthy P2Pool baseline, workers, hashes, and a recent share" "the requested gate cannot safely enable the controller"
        capture_artifacts "xvb-routing" "$OUT_DIR"
        return 0
    else
