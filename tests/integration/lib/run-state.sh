...
assert_running_state() {
    ...
    if [ "$mode" = "local" ]; then
        ...
        # monerod always carries the Tor P2P proxy and Tari's canonical config stays `type = "tor_tcp"`.
        ...
        [ "$tmode" = "local" ] && assert_num_ge "tari canonical config is always Tor (#234)" "$(rx "docker exec tari grep -c '^type = \"tor_tcp\"' /var/tari/config/config.toml 2>/dev/null")" 1 || it_skip_leg "tari canonical config is always Tor (#234)" "tari.mode=$tmode (#1855) — no tari container to inspect" by-design
    ...
    # 8d. The Tor-egress firewall, BOTH directions (#270/#2059). Every other firewall leg here
    # checks STATE, never EFFECT: assert_egress_posture samples the connections the apps CHOSE to
    # make (blind to a fail-open box whose apps are correctly Tor-configured), and
    # verify_tor_egress_firewall compares the installed ruleset to the applier's own render. Until
    # #2059 the only real dial was the opt-out one, asserting the path OPENS — so this suite proved
    # "the opt-out opens it" and never "the default closes it". Both halves now run, each on the row
    # where it is the expected outcome. Full reasoning: tests/os/appliance-egress-leg.sh.
    if [ "$mode" = "local" ]; then
        if [ "$(jq_get "$config" '.network.tor_egress_firewall')" = "false" ]; then
            ...
            # (already in every first-party image — no new tool). Read the CONNECT-phase line, not
            # the HTTP result: a 403/redirect still proves the handshake got through, while a
            # DROPped SYN would just hang to wget's own timeout.
            ...
            #
            # monerod, not xmrig-proxy: the pair needs a WITHIN-ROW control and only a SOCKS-capable
