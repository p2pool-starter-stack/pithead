...
case "$load_src" in *'WORKERS=$((LOAD_BASELINE_COUNT + 1))'*) ;; *) exit 1 ;; esac
...
case "$verify_src" in *'worker_set_ready "$names"'*'all('*'isfinite'*'. >= 0'*) ;; *) exit 1 ;; esac
case "$verify_src" in *'.accepted | tonumber?'*'quote_arg "$clone_shares"'*'[ "$clone_shares" -gt 0 ]'*) ;; *) exit 1 ;; esac
case "$verify_src" in *'sample_load_worker'*'load worker evidence:'*'process_sampled'*'multi-worker-metrics.json'*'worker_set_ready "$names"'*'LOAD_SAW_READY'*'LOAD_SAW_FAILOVER'*'LOAD_SAW_RECOVERY'*) ;; *) exit 1 ;; esac
case "$verify_src" in *'($r | length) >= $workers'*) ;; *) exit 1 ;; esac
...
echo "== worker_set_ready: baseline present + clone online, tolerant of an unrelated extra worker (#1999) =="
...
worker_set_ready "$(printf 'miner-1\nminer-2\npithead-e2e-load-clone\n')" || {
    echo "FAIL: baseline + clone, nothing else, should be ready"
    exit 1
}
worker_set_ready "$(printf 'bench-probe\nminer-1\nminer-2\npithead-e2e-load-clone\n')" || {
    echo "FAIL: an extra worker outside our baseline+clone must not block readiness (a live bench's own pool-health probe can join mid-run)"
    exit 1
}
! worker_set_ready "$(printf 'miner-1\npithead-e2e-load-clone\n')" || {
    echo "FAIL: a DROPPED baseline worker (miner-2) must still be caught"
    exit 1
}
! worker_set_ready "$(printf 'miner-1\nminer-2\n')" || {
...
echo "  ✓ baseline present + clone online passes, an unrelated extra worker does not block it, a dropped baseline or missing clone still fails"
