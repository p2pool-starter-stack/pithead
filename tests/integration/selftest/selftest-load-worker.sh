...
case "$sample_src" in *'docker compose ps --services --status running'*'LOAD_SAW_RECOVERY=1'*'LOAD_SAW_FAILOVER=1'*'LOAD_METRICS_SAMPLED=1'*) ;; *) exit 1 ;; esac
case "$verify_src" in *'[ "$names" = "$expected" ]'*'all('*'. >= 0'*) ;; *) exit 1 ;; esac
case "$verify_src" in *'[ "$clone_shares" -gt 0 ]'*) ;; *) exit 1 ;; esac
