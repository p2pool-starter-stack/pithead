...
case "$load_src" in *'mktemp -d'*'umask 077'*) ;; *) exit 1 ;; esac
case "$load_src" in *'-eq $((LOAD_BASELINE_COUNT + 1))'*) ;; *) exit 1 ;; esac
case "$verify_src" in *'[ "$names" = "$expected" ]'*'accepted | tonumber?'*) ;; *) exit 1 ;; esac
case "$stop_src" in *'/proc/'*'kill -TERM'*'rm -rf'*) ;; *) exit 1 ;; esac
