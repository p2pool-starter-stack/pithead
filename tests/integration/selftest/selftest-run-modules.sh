...
ROOT="$HERE/.."
modules=(run-cli.sh run-matrix.sh run-state.sh run-scenario.sh run-lifecycle.sh run-faults.sh run-hardening.sh run-safety.sh run-uninstall.sh run-rigforge.sh run-rig-control.sh run-rig-reverse.sh run-alert-egress.sh)
...
source "$ROOT/lib/run-uninstall.sh" || exit $?
