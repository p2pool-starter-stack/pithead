...
ROOT="$HERE/.."
modules=(run-cli.sh run-matrix.sh run-state.sh run-scenario.sh run-lifecycle.sh run-faults.sh run-hardening.sh run-safety.sh run-rigforge.sh run-rig-control.sh run-rig-reverse.sh run-alert-egress.sh run-mergemine-submit.sh run-tari-stranded.sh run-mergemine-localnet.sh)
...
expected_functions='usage parse_args print_list push_config env_on_box running_services service_state secret_fingerprint preflight record_manifest run_scenario assert_running_state assert_egress_dial_pair assert_scenario assert_egress_posture assert_xvb_over_tor assert_metrics_via_caddy assert_doctor_ok assert_share_stats_live assert_telemetry_tables_present assert_current_state box_fstype box_avail_gb box_mode assert_release_readiness run_lifecycle kept_data_snapshot_snippet kept_chain_files_snippet run_uninstall_round_trip telemetry_rows_diff _pred_status_down _monerod_is _pred_monerod_missing _pred_monerod_unhealthy _pred_monerod_healthy _pred_proxy_stopped _pred_failover_armed _pred_tor_stopped _pred_tor_healthy fault_node_down fault_unhealthy fault_missing fault_db_readonly fault_firewall_rollback _gf_flows _gf_down fault_firewall_grandfathered_flow fault_firewall_boot_restore fault_tor_down fault_clock_drift fault_disk_enospc _pred_p2pool_peers _pred_dnswatch_listening fault_p2pool_cold_cache_dns _restore_p2pool_peer_lists run_fault_injection _set_env_token _spool_write _uuid4 _wait_control_status _onion_reachable_external _remove_control_units run_hardening run_auth_fail_closed safety_backup safety_restore_exact safety_rollback_if_failed safety_abort_restore arm_safety_abort_restore safety_cleanup restore_baseline summary run_rigforge_integration assert_subnet_live run_subnet_scenario _worker_apply _restore_rig_control_baseline run_rigforge_control _pred_rig_present run_rigforge_reverse _rig_control_apply _rig_control_await _pred_feed_maxt run_rigforge_rollback it_alert_refused _alert_egress_overlay _alert_egress_verdict run_alert_egress_smoke _mm_rows _mm_cleanup run_mergemine_submit tari_ip_of tari_strand_rule tari_strand_remove_all tari_strand_abort tari_health_field _pred_tari_level _pred_tari_restarted run_tari_stranded _mml_fact _mml_target_row _mml_inspect _mml_ip _mml_cleanup run_mergemine_localnet'
