...
ROOT="$HERE/.."
modules=(run-cli.sh run-matrix.sh run-state.sh run-scenario.sh run-lifecycle.sh run-faults.sh run-hardening.sh run-rotate-onion.sh run-safety.sh run-rigforge.sh run-rig-control.sh run-rig-reverse.sh)
...
source "$ROOT/lib/run-rotate-onion.sh" || exit $?
