...
| 2 | Protected release gate | Wired, not yet exercised: `release.sh` and the `main` ruleset both require `bench-ci/tier4` from the pinned App id (provisioned under [#2237](https://github.com/p2pool-starter-stack/pithead/issues/2237)), but the bench has published no tier-4 status yet, so the gate has not run end to end. |
