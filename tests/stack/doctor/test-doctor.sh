...
# component, with the combined requirement (320+200+5+2+1 = 528 GB).
...
assert_contains "single line shows combined ~528 GB" "$out" "needs ~528 GB"
...
assert_contains "small fs warns below need" "$out" "below the ~528 GB"
...
assert_contains "/big groups monero+tari (~520 GB)" "$out" "/big (monero, tari): 600G free — needs ~520 GB"
...
# tari remote drops the ~200 GB Tari share, leaving monero+the small three (320+5+2+1 = 328 GB).
out="$(PATH="$DISK/bin:$PATH" DF_MAP="$one_map" DF_AVAIL_KB=629145600 DF_AVAIL_H=600G \
...
assert_contains "remote tari budget excludes the 200 GB" "$out" "needs ~328 GB"
