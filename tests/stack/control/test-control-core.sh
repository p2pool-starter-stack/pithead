...
# Generated RPC credentials stay blank in the staged intent while preview classifies them for
# confirmation; dry-run validation must not persist generated secrets into that host-only file.
assert_eq "blank-creds preview status" "$(jq -r '.status' "$RESULTS0/$UUID0.json" 2>/dev/null)" "previewed"
assert_eq "blank-creds preview needs confirmation" "$(jq -r '.destructive' "$RESULTS0/$UUID0.json" 2>/dev/null)" "true"
