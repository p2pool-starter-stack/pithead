...
# br- network with an IPv6 subnet, a LAN bridge br0 that carries the default route, and a
...
# reddens the resolved-name rows; failing open on an unreadable interface list reddens the last one.
...
4: br-0a1b2c3d4e5f    inet 172.19.0.1/16 brd 172.19.255.255 scope global br-0a1b2c3d4e5f\       valid_lft forever preferred_lft forever
...
assert_host_local_refused "172.19.0.7" "a container on another bridge network"
...
assert_lan_neighbour_passes_floor "172.20.0.5" "an address just past a bridge's prefix"
...
rm -f "$C/bin/ip" "$C/bin/getent" "$C/ip-fails"
