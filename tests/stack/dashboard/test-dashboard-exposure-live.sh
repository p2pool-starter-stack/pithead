...
echo "== black-box: phase_provision_dashboard_onion_exposure always restores the original config (#2280) =="
# Regression: an early guard that returns before the restore call at the end of the phase would
# leave the appliance permanently onion-enabled. Drive the real phase function with
# approval_capture_restore_snapshot/approval_restore_pending/_ssh stubbed, forcing the guest's
# NETWORK_PREFIX read to come back empty (a live-listener read failure) — the phase must still
# report the failure AND still call the restore, not skip it.
onion_phase_out=$(
    # shellcheck disable=SC1090
    source "$STACK" 2>/dev/null
    set +e
    ...
    _ssh() { # NETWORK_PREFIX grep -> empty, the guard this test targets
        case "$1" in
        *"pithead apply -y"*) return 0 ;; # the onion-enable apply itself succeeds
        *) printf '' ;;
        esac
    }
    ok() { :; }
...
assert_contains "phase still restores the config after the NETWORK_PREFIX guard fires" "$onion_phase_out" "RESTORED"
...
echo "== black-box: phase_provision_dashboard_onion_exposure restores even when the apply itself fails (#2280) =="
onion_apply_fail_out=$(
    # shellcheck disable=SC1090
    source "$STACK" 2>/dev/null
    set +e
    approval_capture_restore_snapshot() { return 0; }
