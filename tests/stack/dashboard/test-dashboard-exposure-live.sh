...
render_exposure_caddy() { # <expose-public>
    (
        ...
        DASHBOARD_SECURE=true HOST_IP=pithead.local NETWORK_PREFIX="$network_prefix" DASHBOARD_AUTH_HASH_B64="" \
...
safe_ss="LISTEN 0 4096 $lan:443 0.0.0.0:* users:((\"caddy\",pid=1,fd=7))
LISTEN 0 4096 [$ula]:443 [::]:* users:((\"caddy\",pid=1,fd=8))"
public_ss="LISTEN 0 4096 [$global]:443 [::]:* users:((\"caddy\",pid=1,fd=7))"
wild_ss='LISTEN 0 4096 [::]:443 [::]:* users:(("caddy",pid=1,fd=7))'
...
assert_contains "live exposure verdict fires on the public-bind mutation" \
    "$(dashboard_exposure_verdict "$open_caddy" "$wild_ss" 0 '{}' "$lan" "$ula" "$global" pithead.local || true)" "global address"
