...
# shellcheck source=tests/stack/test-wallet-rpc.sh disable=SC2015
_d0=$((PASS + FAIL)) && source "$HERE/test-wallet-rpc.sh" && domain_ran test-wallet-rpc.sh "$_d0" "$?" || domain_ran test-wallet-rpc.sh "$_d0" "$?"
