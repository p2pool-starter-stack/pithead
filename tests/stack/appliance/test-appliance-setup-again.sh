...
printf 'stale\n' >"$HCSB/spool/.handoff.json.$$"
chmod 644 "$HCSB/spool/.handoff.json.$$"
printf '{"role":"rig","token":"tok-2"}' | run_sourced "$HCSB" write_handoff_card "$HCSB/spool"
assert_rc "a stale permissive temp is corrected -> rc 0" "$?" "0"
assert_eq "the stale temp is 600 when published" "$(stat -c '%a' "$HCSB/spool/handoff.json")" "600"
chmod 644 "$HCSB/spool/handoff.json"
