...
# Lifecycle suite split at its existing recovery, compose-up retry, lock primitive, verb wiring,
# and appliance seams.
# shellcheck source=tests/stack/lifecycle/recovery.sh
source "$HERE/lifecycle/recovery.sh" || return $?
# shellcheck source=tests/stack/lifecycle/compose-up-retry.sh
source "$HERE/lifecycle/compose-up-retry.sh" || return $?
