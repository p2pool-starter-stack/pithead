...
EOF
chmod +x "$WT/bin/df"
# 600 GiB clears the 528 GB the whole stack with a bundled Tari node needs; 100 GiB does not.
...
# Monero node keeps its chain on another host, so its 320 GB drops out of the comparison.
