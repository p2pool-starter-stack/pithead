...
- **The appliance does not boot with Secure Boot on, and nothing signs the chain (#2187).** The
  ...
  reached. It is not: the defined-but-not-booted row records the deferred state without failing
  unrelated KVM evidence. The image
  ...
  the measured outcome plainly, and reports signing works when a signed chain exists. A guest that cannot even be DEFINED — no OVMF
