...
# rc 137 — no message, nothing naming memory, the shellcheck line just stops. It lints one file
# per process, four at a time (#2632): the largest process (the generated pithead) peaks at
# 2.2 GiB and the four largest files sum to 5.8 GiB, the worst case when they overlap. Warn
# rather than refuse: every other target needs a fraction of this, and blocking test-fakes over
# lint's ceiling would be its own kind of wrong.
LINT_PEAK_GIB=6
vm_bytes="$($ENGINE info --format '{{.MemTotal}}' 2>/dev/null || echo 0)"
if [ "${vm_bytes:-0}" -gt 0 ] && [ "$vm_bytes" -lt $((LINT_PEAK_GIB * 1024 * 1024 * 1024)) ]; then
    printf 'test-container: the engine has %.1f GiB; lint-sh runs four shellchecks that can need 5.8 GiB together.\n' \
